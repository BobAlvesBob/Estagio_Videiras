...
O primeiro modelo foi treinado com recurso a um dataset público disponível na
plataforma Roboflow, oriundo de investigações realizadas em vinhas espanholas.
Este dataset apresenta imagens de maior qualidade fotográfica e anotações
mais uniformes, constituindo uma boa base para uma primeira validação da
abordagem.

As etiquetas originais foram revistas e refinadas, e o dataset foi integrado
com amostras adicionais para aumentar a diversidade morfológica das instâncias
de gomos.
...
A definição das classes resultou de uma análise iterativa das dificuldades
encontradas nas primeiras sessões de anotação. A necessidade de incluir
elementos como troncos e outros objetos (arames, postes) surgiu da observação de que o modelo tendia a
confundir esses elementos com sarmentos e gomos, gerando falsos positivos.
A Tabela~\ref{tab:classes} descreve cada classe e os critérios adoptados.

\begin{table}[H]
  \centering
  \caption{Classes de anotação e respetivos critérios}
  \label{tab:classes}
  \begin{tabularx}{\textwidth}{l p{3.5cm} X}
    \toprule
    \textbf{Classe}    & \textbf{Descrição}              & \textbf{Critério de anotação} \\
    \midrule
    \texttt{gomo}      & Gomo visível no sarmento        & Bounding box ajustada ao contorno do gomo, mesmo que parcialmente oclusos. \\[0.4em]
    \texttt{sarmento}  & Ramo anual que nasce do cordão  & Caixa delimitadora ao longo do sarmento visível; sarmentos parcialmente oclusos são anotados se pelo menos 30\% estiver visível. \\[0.4em]
    \texttt{tronco}    & Tronco principal da planta      & Incluído para evitar confusão com sarmentos de fundo. \\[0.4em]
    \texttt{objeto}     & Elementos não classificados (arames, fios, postes) & Incluído para reduzir falsos positivos em estruturas que não são gomo, sarmento ou tronco. \\
    \bottomrule
  \end{tabularx}
\end{table}
